...
export class UserNotProvisionedError extends Error {
  constructor() {
    super("Account is not provisioned. Contact an administrator.");
  ...
  if (existing) {
    return prisma.user.update({
      ...
      data: {
        ...
        studentProfile:
          existing.role === Role.STUDENT && !existing.studentProfile
            ? { create: { githubUsername: existing.githubUsername ?? undefined } }
            : undefined,
        leaderboard:
          existing.role === Role.STUDENT && !existing.leaderboard
            ? { create: {} }
            : undefined,
      },
      include: { profile: true, studentProfile: true, leaderboard: true },
